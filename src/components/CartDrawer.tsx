...
export function CartDrawer() {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed top-0 right-0 h-full w-full sm:w-[420px] bg-bg-white z-[101] shadow-2xl transition-transform duration-300 ease-out flex flex-col ${
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <Link href="/temas" onClick={closeCart} className="btn-primary">
                Explorar temas
              </Link>
            </div>
          ) : (
            <ul className="divide-y divide-border-light">
              {items.map((item) => (
                <li key={item.itemId} className="p-4 flex gap-4">
                  <div className="relative w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-bg">
                    {item.image ? (
                      <Image src={item.image} alt={item.themeName} fill sizes="64px" className="object-cover" />
                    ) : (
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center gap-1 border border-border rounded-lg">
                        <button
                          ...
                          className="px-2 py-1 hover:bg-primary-bg rounded-l-lg text-text-secondary"
                        >
                          -
                        </button>
                        ...
                        <button
                          onClick={() => updateQuantity(item.itemId, item.quantity + 1)}
                          className="px-2 py-1 hover:bg-primary-bg rounded-r-lg text-text-secondary"
                        >
                          +
                        </button>
                      </div>
        ...
        {items.length > 0 && (
          <div className="border-t border-border-light p-6 space-y-4">
            ...
            <p className="text-xs text-text-tertiary">Envio calculado en el checkout. Digitales sin costo de envio.</p>
